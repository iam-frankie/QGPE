...
//                .flatMap(new GlobalMergerFunction(M));
        ...
        DataStream<Cluster> clusterStream = gDBSCAN.gridDBSCAN_Launcher(snapshotStream, geoHashPrecision, M, epsilon, minPts, ConversionFactor, TwindowSize, parallelism, isTest, SlidingWinndowSize);

        int finalGeoHashPrecision = geoHashPrecision;
        DataStream<SubMap> ces = clusterStream
//                .map(new MapFunction<Cluster, Cluster>() {
//                    @Override
//                    public Cluster map(Cluster cluster) throws Exception {
////                        System.out.println(cluster.ptIDs);
//                        return cluster;
//                    }
//                })
                .keyBy(cluster -> cluster.getClusterID().substring(0, finalGeoHashPrecision))     // partition by ptid
